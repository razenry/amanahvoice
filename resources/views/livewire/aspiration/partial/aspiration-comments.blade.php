...
<article
    ...
    x-data="{ showDeleteConfirm: false, showReplies: false, isHovered: false }"
...
>
            ...
            <div class="min-w-0 flex-1">
                <div class="flex flex-wrap gap-1 md:gap-2 items-center">
                    ...
                    @if($comment->user_id === $comment->aspiration->user_id)
                        <span class="text-xs font-medium bg-blue-50 dark:bg-blue-900 text-blue-600 dark:text-blue-100 rounded px-1.5 py-0.5">
                            Penulis
                        </span>
                    @endif
                </div>
                <div class="text-xs text-gray-500 dark:text-gray-400">
                    ...
                    @if($isReply)
                        ...
                        <span class="block md:inline mt-0.5 md:mt-0 md:ml-1">{{ __('Membalas') }} {{ $comment->parent->user->name }}</span>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center mt-2 md:mt-3 space-x-3 md:space-x-4">
            @if(!$isReply)
                <button type="button" wire:click="reply({{ $comment->id }})"
                    ...
                    <svg class="w-3.5 h-3.5 md:w-4 md:h-4 mr-1 md:mr-1.5" fill="currentColor" viewBox="0 0 20 18">
                        <path d="M18 0H2a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h2v4a1 1 0 0 0 1.707.707L10.414 13H18a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2Z"/>
                    </svg>
            ...
            @if($comment->replies->count())
                <button @click="showReplies = !showReplies"
                    ...
                    <svg class="w-3.5 h-3.5 md:w-4 md:h-4 mr-1 md:mr-1.5 transition-transform duration-200" :class="{ 'rotate-180': showReplies }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                    <span x-text="showReplies ? 'Sembunyikan' : `({{ $comment->replies->count() }}) Balasan`" class="whitespace-nowrap"></span>
                </button>
            @endif
        </div>
    </div>
    ...
    @if($comment->replies->count())
        <div x-show="showReplies" x-collapse x-cloak class="mt-2 md:mt-3 pl-4 md:pl-6 border-l border-gray-100 dark:border-gray-700">
